<?php
  ...
  if(checkUserAndProd($id_user, $id_prod)){
    $db = null;
    echo json_encode(array('msg'=>"Il prodotto $_POST[name] è già presente nella wishlist."));
  } else {
    $sql = "INSERT INTO wish (id_user, id_product)
    VALUES ('$id_user', '$id_prod')";
    $db->exec($sql);
    $db = null;
    echo json_encode(array('msg'=>"Il prodotto $_POST[name] è stato aggiunto alla wishlist."));
  }
